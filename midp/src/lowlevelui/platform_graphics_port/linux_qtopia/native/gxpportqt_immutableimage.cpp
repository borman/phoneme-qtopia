...
#warning "Много херни. Требуется ревизия"
     ...
     jint x_dest, jint y_dest)
{
    ...
    QPainter *gc = JGraphics::setupGC(-1, -1, clipRect,
                                      qpixmapDst,
                                      0);
...
//                     }
                ...
                *ret_length = offsetof(imgdcd_image_buffer_raw, data)
                ...
                else
                {
                    ...
                    dataBuffer->hasAlpha = (unsigned int)true;
    ...
    (gxpport_image_native_handle immutableImagePtr)
{
    ...
    *(immutableImage->qpixmap) = QPixmap::fromImage(*immutableImage->qimage);
