...
LFPKeyMap *LFPKeyMap::jkeymap = NULL;

LFPKeyMap *LFPKeyMap::instance()
{
  if (!jkeymap)
    init();
  return jkeymap;
...
{
  if (!jkeymap)
    jkeymap = new LFPKeyMap();
}

void LFPKeyMap::destroy()
{
  if (jkeymap)
  {
    delete jkeymap;
...
bool LFPKeyMap::map(int qtkey, const QString &unicode, int &javakey)
{
