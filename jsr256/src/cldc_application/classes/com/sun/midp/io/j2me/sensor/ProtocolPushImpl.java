...
public class ProtocolPushImpl extends ProtocolPush {
    ...
    public void registerConnection(MIDletSuite midletSuite, String connection, 
        ...
        SensorUrl su = SensorUrl.parseUrlPush(connection);  // throws IllegalArgumentException
        Sensor[] sensors = SensorRegistry.findSensors(su);
        if (sensors == null || sensors.length != 1) {
            throw new IllegalArgumentException("No or too many sensors");
        }

        Protocol.checkPrivateProtectedPermission(sensors[0]);
    }
}
