...
#define DEFAULT_PROVIDER 1	/* NULL Criteria Provider is Prov_N2 */

javacall_utf16 provider_name[MAX_PROVIDERS][10] = {{'P','r','o','v','_','N','1',0},{'P','r','o','v','_','N','2',0}};
javacall_utf16 orientation_provider_name[] = {'O','r','i','e','n','t','a','t','i','o','n',0};
int provider_name_len[MAX_PROVIDERS] = {7, 7};
int orientation_provider_name_len = 11;

int provider_descr[MAX_PROVIDERS] = {1, 2};
int orientation_provider_descr = 5;
...
javacall_location_orientation orientation_data = 
	{0.0, 0.0, 0.0, JAVACALL_FALSE};

typedef struct _proximity_data {
	javacall_handle provider_handle;
	double longitude;
	double latitude;
	float radius;
	struct _proximity_data *next;
} proximity_data;

proximity_data *providers_proximity_head[MAX_PROVIDERS] = {NULL, NULL};

/************************************************************************/
/* Extract the event data using LIME from:                              */
/* public String LocationEventGenPanel.getEventData(String code)        */
/*                                                                      */
/* The result should be parsed and fitted into the right structs        */
/* It is up to the following functions to free this return value        */
/************************************************************************/
char *ExtractEventData(javacall_utf16_string event_name) {
    static LimeFunction *f = NULL;

    unsigned short *ret = NULL;
    int retLen = 0;
    int eventLen = 0;
    int unicodeEventSize = 0;
    char *event_data = NULL;
    int event_data_length = 0;
    char *return_event_data = NULL;

    if (f == NULL) {
            f = NewLimeFunction("com.sun.kvem.midp",
                                "EventBridge", "getExternalEventData");
    }

    eventLen = wcslen(event_name);
    f->call(f, &ret, &retLen, event_name, eventLen);

    if (0 == retLen)
            return NULL;

    event_data = malloc(sizeof(char)*(retLen+1));
    if (NULL == event_data)
            return NULL;
    memset(event_data,0,retLen+1);
    wcstombs(event_data, ret, retLen);
    event_data[retLen] = NULL;
    return event_data;
}

void ParseLocationData(char *data) {
	char *latS, *lonS, *altS, *speedS, *courseS;
	double lat, lon, alt, speed, course;
	javacall_location_location currentLocation;

	/* check for errors in the extraction process */
	if (data == NULL)
		javautil_debug_print (JAVACALL_LOG_ERROR, "location",
        "ERROR: event data wasn't extracted (insufficient memory?)");

	/* now we have the location string as C string in locationData	*
	 * we need to parse it and convert it to int variables	        */
	latS = strtok(data, ";");
	lonS = strtok(NULL, ";");
	altS = strtok(NULL, ";");
	speedS = strtok(NULL, ";");
	...
	currentLocation = providers_location[DEFAULT_PROVIDER];
	...
	currentLocation.method = 0;
	currentLocation.timestamp = time(0);

	providers_location[DEFAULT_PROVIDER] = currentLocation;

	free(data);
}

void ParseOrientationData(char *data) {
	char *azimuthS, *magneticS, *pitchS, *rollS;
	double azimuth, pitch, roll;
	javacall_bool magnetic;

	/* handle the case the Orientation is unsupported or unavailable */
	if (!strcmp(data,"null"))
		/* Orientation is not supported */
	if (!strcmp(data,"false"))
		/* Orientation is unavailable */

	azimuthS = strtok(data, ";");
	magneticS = strtok(NULL, ";");
	pitchS = strtok(NULL, ";");
	rollS = strtok(NULL, ";");

	azimuth = atof(azimuthS);
	pitch = (strcmp("null", pitchS)?atof(pitchS):0.0);
	roll = (strcmp("null", rollS)?atof(rollS):0.0);
	magnetic = (strcmp(magneticS, "true")?JAVACALL_FALSE:JAVACALL_TRUE);

	orientation_data.compassAzimuth = azimuth;
	orientation_data.isMagnetic = magnetic;
...
void ParseAccuracyData(char *data) {
	char *horS, *verS;
	double hor, ver;
	...
	/* In the emulator, ACCURACY event is sent right after a LOCATION event,	*/
	/* so we're updating the last location object available.		*/
	providers_location[DEFAULT_PROVIDER].horizontalAccuracy = hor;
	providers_location[DEFAULT_PROVIDER].verticalAccuracy = ver;

	free(data);
}

/**
 * Checks for proximity of a coordinate and notify it
 */
void HandleProximityLocationEvent() {
	proximity_data *data = providers_proximity_head[DEFAULT_PROVIDER];
	double distance;
	double lat1, lon1, lat2, lon2;

	lat1 = providers_location[DEFAULT_PROVIDER].latitude;
	lon1 = providers_location[DEFAULT_PROVIDER].longitude;
	...
	while (data != NULL)
	{
		...
		/* calculate distance between current location	*/
		/* the registered proximity coordinates			*/
		distance = 2*asin(sqrt(pow((sin((lat1-lat2)/2)),2) +
                 cos(lat1)*cos(lat2)*pow((sin((lon1-lon2)/2)),2)));

		if (distance <= data->radius)
			javanotify_location_proximity(data->provider_handle,
				data->latitude, data->longitude, data->radius,
				&(providers_location[DEFAULT_PROVIDER]), JAVACALL_OK);
...
/**
 * Fires monitoring state change as a result of a state change
 */
void HandleLocationProviderStateEvent(int state){
        char ch_state = state + '0';
        javacall_handle fd;  
        int flag = JAVACALL_FILE_O_RDWR;

        if (javacall_file_exist(provider_name[DEFAULT_PROVIDER], provider_name_len[DEFAULT_PROVIDER]) == JAVACALL_FAIL) {
            flag |= JAVACALL_FILE_O_CREAT;
        }
        if (javacall_file_open(provider_name[DEFAULT_PROVIDER], provider_name_len[DEFAULT_PROVIDER], flag, &fd) == JAVACALL_OK) {
            javacall_file_write(fd, (const unsigned char *)&ch_state, sizeof(ch_state));
...
static DWORD WINAPI open_provider(void* pArg)
{
    ...
    if (javacall_file_exist(provider_name[ind], provider_name_len[ind]) == JAVACALL_FAIL) {
        if (javacall_file_open(provider_name[ind], provider_name_len[ind], JAVACALL_FILE_O_CREAT | JAVACALL_FILE_O_RDWR, &fd) == JAVACALL_OK) {
...
/**
 * Open Orientation Provider thread
 */
static DWORD WINAPI open_orientation_provider(void* pArg)
{
    Sleep(1000);
    javanotify_location_event( JAVACALL_EVENT_LOCATION_OPEN_COMPLETED, (javacall_handle)orientation_provider_descr, JAVACALL_OK);
    return 0;
}
 ...
 * Update Location thread
 */
static DWORD WINAPI update_orientation(void* pArg)
{
    Sleep(1000);
    javanotify_location_event( JAVACALL_EVENT_LOCATION_ORIENTATION_COMPLETED, (javacall_handle)orientation_provider_descr, JAVACALL_OK);
    return 0;
}
    ...
    switch(property) {
        ...
        case JAVACALL_LOCATION_ORIENTATION_LIST:
            for(j=0; j<orientation_provider_name_len; j++) {
                outPropertyValue[k++] = orientation_provider_name[j];
            }
            outPropertyValue[orientation_provider_name_len] = 0;
        return JAVACALL_OK;
    default:
...
javacall_result javacall_location_provider_open(
    ...
    if(found == JAVACALL_TRUE) {
        ...
        *pProvider = (javacall_handle)provider_descr[i];
        ...
        /* Verify Orientation Provider */
        for(j=0; j<orientation_provider_name_len; j++) {
            if(name[j] != orientation_provider_name[j]) {
                break;
            }
        }
        if((j == orientation_provider_name_len) && (name[j] == 0)) {
            *pProvider = (javacall_handle)orientation_provider_descr;
            CreateThread(NULL, 0, open_orientation_provider, NULL, 0, NULL);
            return JAVACALL_WOULD_BLOCK;
        }
    }
    return JAVACALL_FAIL;
...
javacall_result javacall_location_provider_close(
    ...
    if(found == JAVACALL_TRUE) {
        if(JAVACALL_FALSE == provider_opened[i]) {
            javacall_print("the provider is already closed\n");
        } else {
            provider_opened[i] = JAVACALL_FALSE;
        }
    } else {
        return JAVACALL_FAIL;
    }
...
javacall_result javacall_location_provider_state(
    ...
    if(found == JAVACALL_TRUE) {
        if (javacall_file_open(provider_name[i], 7, JAVACALL_FILE_O_RDWR, &fd) == JAVACALL_OK) {
...
javacall_result /*OPTIONAL*/ javacall_location_get_addressinfo(
    ...
    if((int)provider == provider_descr[0]) {    
        if(*pAddresInfoFieldNumber >= 2){
            fields[0].fieldId = JAVACALL_LOCATION_ADDRESSINFO_COUNTRY;
            fields[0].data[0] = 'R';
            fields[0].data[1] = 'u';
            fields[0].data[2] = 's';
            fields[0].data[3] = 's';
            ...
            fields[0].data[5] = 'a';
            fields[0].data[6] = 0;
            fields[1].fieldId = JAVACALL_LOCATION_ADDRESSINFO_CITY;
            fields[1].data[0] = 'S';
            fields[1].data[1] = '-';
            fields[1].data[2] = 'P';
            fields[1].data[3] = 'e';
 ...
 *
 * @retval JAVACALL_OK          success
 * @retval JAVACALL_WOULD_BLOCK javanotify_location_event needs to be called
 *                              to notify completion. The type of event will be
 *                              JAVACALL_EVENT_LOCATION_ORIENTATION_COMPLETED
 * @retval JAVACALL_FAIL        if gets a invalid orientation or other error
 */
javacall_result /*OPTIONAL*/ javacall_location_orientation_update(
        javacall_handle provider) {
    CreateThread(NULL, 0, update_orientation, NULL, 0, NULL);
    return JAVACALL_WOULD_BLOCK;
}

/**
 * Gets a pending orientation operation.
 *
 * @see javanotify_location_event
 *
 * @param provider handle of a location provider
...
javacall_result /*OPTIONAL*/ javacall_location_orientation_get(
    ...
    pOrientationInfo->compassAzimuth = orientation_data.compassAzimuth;
    pOrientationInfo->pitch= orientation_data.pitch;
    pOrientationInfo->roll= orientation_data.roll;
