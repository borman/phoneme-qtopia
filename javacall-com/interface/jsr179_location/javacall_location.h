 ...
 *
 * For the specified location provider, 
 *    the following infomation should be provided:
 ...
 *  - provide speed and course information
 *  - provide address information related to the location.
 ...
 * JAVACALL_WOULD_BLOCK. When the operation has been completed, an event
 * is required to be sent from Platform. See section on event for related
 * information.
 *
 ...
 */
typedef enum {
    /** no requirements for the power consumption */
    JAVACALL_LOCATION_NO_IDEA               = 0,
    /** low power consumption allowed */
 ...
 */
typedef enum {
    /** "application/X-jsr179-location-nmea" mime type */
    JAVACALL_LOCATION_EXTRAINFO_NMEA               = 0,
    /** "application/X-jsr179-location-lif" mime type */
 ...
 */
typedef enum {
    /** Satellites location method */
    JAVACALL_LOCATION_MTE_SATELLITE = 0x00000001,
    /** Time Difference location method */
    JAVACALL_LOCATION_MTE_TIMEDIFFERENCE = 0x00000002,
    /** Time of Arrival (TOA) location method */
    JAVACALL_LOCATION_MTE_TIMEOFARRIVAL = 0x00000004,
    /** Cell-ID for cellular location method */
    JAVACALL_LOCATION_MTE_CELLID = 0x00000008,
    /** Short-range positioning (Bluetooth LP) location method */
    JAVACALL_LOCATION_MTE_SHORTRANGE = 0x00000010,
    /** Angle of Arrival location method for cellular/terrestrial RF system. */
...
typedef enum {
   ...
   /** Location Provider is temporary unavailable */
...
#define JAVACALL_LOCATION_FLOAT_NAN \
        (((union { unsigned __l; float __d; })  { __l: 0x7fc00000U }).__d)
#else
 ...
 * 
 * @brief The information of location provider including the criteria
 *        information which the location provider could meet.
 */
typedef struct {
     ...
     */
    int defaultMaxAge;
    /** The default listener interval between cyclic location updates
     * in milliseconds. 
     */ 
    int defaultInterval;
    ...
    /** The recommended interval for querying provider's state
     *  in milliseconds. 
     */
 ...
 */
typedef struct {
    /** Address Info fieldId (see javacall_location_addressinfo_field) */
    javacall_location_addressinfo_field fieldId;
    /** Address Info data */
 ...
 */
typedef struct {
    /** whether a valid location with coordinates or an invalid one,
     *  invalid may have the extra information for the reason  
     */ 
    ...
    double longitude;           
    /** defined as height in meters above the WGS84 ellipsoid,
     *  JAVACALL_LOCATION_FLOAT_NAN if unknown 
     */
    float altitude;      
    /** in meters(1-sigma standard deviation),
     *  JAVACALL_LOCATION_FLOAT_NAN if unknown 
     */
    ...
    /** 0 if none or the length of the extra infomation string
     *  which includes NULL termination 
     */
    int extraInfoSize;          
...
typedef struct {
    ...
    /** Pitch in degree within [-90.0, 90.0],
     *  JAVACALL_LOCATION_FLOAT_NAN if unknown 
     */
    float pitch;            
    /** Roll in degree within [-180.0, 180.0),
     *  JAVACALL_LOCATION_FLOAT_NAN if unknown 
     */
    float roll;
    /** If true, the compassAzimuth and pitch are relative to the magnetic
     *  field of the Earth. 
     *  If false, the compassAzimuth is relative to true north and pitch
 ...
 *
 * If there are more than one items in a property string, 
 ...
 *   The lists of orientation providers. An empty string means that orientation
 *   is not supported.
 *
 ...
 *
 * The valid provider name is listed in JAVACALL_LOCATION_PROVIDER_LIST 
 * property.
 * 
 * @param name of the location provider, NULL implies the default location
 *        provider
 * @param pInfo  the information of the location provider
 * 
 * @retval JAVACALL_OK                success
 * @retval JAVACALL_INVALID_ARGUMENT  if the given name of location provider
 *                                    is not found.
 ...
 * The name of the location provider is in 
 * JAVACALL_LOCATION_PROVIDER_LIST property.
 ...
 * This function only get the current state and is intended
 * to return it quickly. 
 *
 ...
 *
 * This function only requests location update and is intended
 * to return it quickly. The location update will be get through
 * javanotify_location_event() with JAVACALL_EVENT_LOCATION_UPDATE_ONCE type.
 * This function will not be called again before javanotify_location_event
 * is called for the previous request completion. If timeout expires before
 * obtaining the location result, javanotify_location_event() will be called
 * with JAVACALL_LOCATION_RESULT_TIMEOUT reason.
 *
 * @see javanotify_location_event
 *
 * @param provider handle of the location provider
 ...
 */
javacall_result javacall_location_update_set(javacall_handle provider, 
                                             javacall_int64 timeout);

/**
 * Cancels the current location acquisition.
 * 
 * This function will incur calling javanotify_location_event() with
 * JAVACALL_LOCATION_RESULT_CANCELED reason.
 *
 * @see javanotify_location_event
 *
 * @param provider handle of the location provider
 ...
 *
 * The location update will be notified through javanotify_location_event()
 * with JAVACALL_EVENT_LOCATION_UPDATE_ONCE type. 
 *
 * @see javanotify_location_event
 *
 * @param provider handle of the location provider
 ...
 *
 * @param provider handle of the location provider
 * @param pAddresInfoFieldNumber used for both input and output number
 *        of array elements. 
 ...
 *
 * @retval JAVACALL_OK          success
 * @retval JAVACALL_WOULD_BLOCK javanotify_location_event needs to be called to
 *                              notify completion. The type of event will be
 *                              JAVACALL_EVENT_LOCATION_ORIENTATION_COMPLETED
 ...
 * javanotify_location_proximity() is called only once when the terminal enters
 * the proximity of the resigered coordinate. 
 * The monitoring state change is reported as well through the operation_result
 * parameter of javanotify_location_proximity(). 
 *
 * @param provider handle of the location provider
 * @param latitude of the location to monitor
 * @param longitude of the location to monitor
 * @param proximityRadius the radius in meters that is considered to be
 *        the threshold for being in the proximity of the specified location.
 *
 * @retval JAVACALL_OK               success
 * @retval JAVACALL_FAIL             the already monitored location is provided,
 *                                   or if the platform does not have resources
 *                                   to add a new location to be monitored.
 * @retval JAVACALL_INVALID_ARGUMENT if latitude or longitude are not within 
 *                                   the valid range. if proximityRadius < 0
 * 
 */
