...
static pcsl_string* get_list_path(jsr75_pim_type type, jchar* name) {
    ...
    if (PCSL_STRING_OK != (res = pcsl_string_convert_from_utf8(midp_root, strlen((char*)midp_root), path1))) {
        return NULL;
    }
    for (list_index = 0;
...
static char* get_item_path(pim_opened_list *list, char* item_name) {
    ...
    if (file_path) {
        res = pcsl_string_convert_to_utf8(&list_path, (jbyte*)file_path, lp_len + 1, &fp_len);
...
jsr75_pim_list_get_next_item(
  ...
  jsr75_handle /* OUT */ *itemHandle
    ...
    *itemHandle = NULL;
    ...
    res = pcsl_string_convert_to_utf8(&fname, (jbyte*)item_name, item_name_len, &len);
    pcsl_string_free(&fname);
    if (PCSL_STRING_OK != res) {
        free(item_name);
        return JSR75_STATUSCODE_FAIL;
 ...
 * JSR75_PIM_INVALID_ID should be set for each member that is not in use.
 *
