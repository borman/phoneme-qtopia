...
/**
 * Field IDs of the <tt>com.sun.j2me.location.CriteriaImpl</tt> class
 * The fields initialized at startup and used to speedup access to the fields
 */
typedef struct{
    jfieldID costAllowed;
    jfieldID altitudeRequired;
    jfieldID addressInfoRequired;
    jfieldID speedCourceRequired;
    jfieldID powerConsumption;
    jfieldID horizontalAccuracy;     
    jfieldID verticalAccuracy;       
    jfieldID responseTime;    
    jfieldID providerName;
}CriteriaImplFieldIDs;
static CriteriaImplFieldIDs criteriaImplFieldID;
...
KNIDECL(com_sun_j2me_location_PlatformLocationProvider_getBestProviderByCriteriaImpl) {
    javacall_result res;
    javacall_utf16 listOfProviders[JAVACALL_LOCATION_MAX_PROPERTY_LENGTH];
    jboolean ret = KNI_FALSE;
    javacall_location_criteria criteria_info;
    ...
    KNI_GetParameterAsObject(1, criteria);

    KNI_GetObjectClass(criteria, class_obj);
    criteria_info.costAllowed = KNI_GetBooleanField(criteria, 
        criteriaImplFieldID.costAllowed);
    criteria_info.altitudeRequired = KNI_GetBooleanField(criteria, 
        criteriaImplFieldID.altitudeRequired);
    criteria_info.addressInfoRequired = KNI_GetBooleanField(criteria, 
        criteriaImplFieldID.addressInfoRequired);
    criteria_info.speedCourceRequired = KNI_GetBooleanField(criteria, 
        criteriaImplFieldID.speedCourceRequired);
    criteria_info.powerConsumption = KNI_GetIntField(criteria, 
        criteriaImplFieldID.powerConsumption);
    criteria_info.horizontalAccuracy = KNI_GetIntField(criteria, 
        criteriaImplFieldID.horizontalAccuracy);
    criteria_info.verticalAccuracy = KNI_GetIntField(criteria, 
        criteriaImplFieldID.verticalAccuracy);
    criteria_info.responseTime = KNI_GetIntField(criteria, 
        criteriaImplFieldID.responseTime);

    res = javacall_location_provider_get_by_criteria(&criteria_info, listOfProviders);

    if (res == JAVACALL_NOT_IMPLEMENTED) {
        ret = KNI_FALSE;
    } else {
        if (res == JAVACALL_OK) {
            if (listOfProviders[0] == (javacall_utf16)0) {
                KNI_ReleaseHandle(name);
            } else {
                jsrop_jstring_from_utf16_string(KNIPASSARGS listOfProviders, name);
            }
            KNI_SetObjectField(criteria, criteriaImplFieldID.providerName, name);
            ret = KNI_TRUE;
        }
    }
...
javacall_result open_provider(const javacall_utf16_string name, 
           /*OUT*/ javacall_handle* pProvider, /*OUT*/ProviderInfo **pInfo) {
    javacall_result res = javacall_location_provider_open(name, pProvider);
    switch(res) {
        case JAVACALL_OK:
...
KNIEXPORT KNI_RETURNTYPE_INT
    ...
    JAVACALL_LOCATION_ASYNC_EXEC(
        res,
        open_provider(name, &provider, &pInfo),
        provider, JAVACALL_EVENT_LOCATION_OPEN_COMPLETED, pInfo
    );
    switch (res) {
        ...
        case JAVACALL_FAIL:
            /* fail */
            KNI_ThrowNew(jsropIOException, "open failed");
            if (pInfo != NULL) {
...
KNIEXPORT KNI_RETURNTYPE_VOID
    KNIDECL(com_sun_j2me_location_PlatformLocationProvider_setUpdateIntervalImpl) {

    ProviderInfo *pInfo = (ProviderInfo *)KNI_GetParameterAsInt(1);
    jint interval = KNI_GetParameterAsInt(2);
    javacall_location_provider_set_update_interval(pInfo->id, interval);
...
KNIEXPORT KNI_RETURNTYPE_BOOLEAN
    ...
    if(pInfo != NULL) {
        ...
        JAVACALL_LOCATION_ASYNC_EXEC(
            result,
            javacall_location_update_set(provider, timeout),
            provider, JAVACALL_EVENT_LOCATION_UPDATE_ONCE, pInfo
        );
...
KNIEXPORT KNI_RETURNTYPE_VOID
    ...
    if(pInfo != NULL) {
        JAVACALL_LOCATION_ASYNC_EXEC(
            res,
            javacall_location_update_cancel(pInfo->id),
            pInfo->id, JAVACALL_EVENT_LOCATION_UPDATE_ONCE, pInfo
        );
        switch (res) {
            case JAVACALL_OK:
            case JAVACALL_FAIL:
            ...
            case JAVACALL_INVALID_ARGUMENT:
                /* wrong provider name */
                KNI_ThrowNew(jsropIllegalArgumentException, "wrong provider");
                break;
            default:
                break;
        }
...
KNIEXPORT KNI_RETURNTYPE_INT
    ...
    javacall_int32 length = 0;
        ...
        if (length > 0) {
            ...
            JAVACALL_LOCATION_ASYNC_EXEC(
                res,
                javacall_location_provider_open(listOfOrientationProviders, &provider),
                provider, JAVACALL_EVENT_LOCATION_OPEN_COMPLETED, provider
            );
...
KNIEXPORT KNI_RETURNTYPE_BOOLEAN
    ...
    provider = (ProviderInfo *)KNI_GetParameterAsInt(1);
    KNI_StartHandles(1);
    ...
    JAVACALL_LOCATION_ASYNC_EXEC(
        status,
        javacall_location_orientation_update(provider),
        provider, JAVACALL_EVENT_LOCATION_ORIENTATION_COMPLETED, provider
    );
...
/* JAVADOC COMMENT ELIDED */
KNIEXPORT KNI_RETURNTYPE_VOID
    KNIDECL(com_sun_j2me_location_CriteriaImpl_initNativeClass) {
    ...
    if(!KNI_IsNullHandle(class_obj)) {
        criteriaImplFieldID.costAllowed = 
            KNI_GetFieldID(class_obj,"costAllowed","Z");
        criteriaImplFieldID.altitudeRequired = 
            KNI_GetFieldID(class_obj,"altitudeRequired","Z");
        criteriaImplFieldID.addressInfoRequired = 
            KNI_GetFieldID(class_obj,"addressInfoRequired","Z");
        criteriaImplFieldID.speedCourceRequired = 
            KNI_GetFieldID(class_obj,"speedCourceRequired","Z");
        ...
        criteriaImplFieldID.verticalAccuracy = 
            KNI_GetFieldID(class_obj,"verticalAccuracy","I");
        criteriaImplFieldID.responseTime = 
            KNI_GetFieldID(class_obj,"responseTime","I");

        criteriaImplFieldID.providerName = 
            KNI_GetFieldID(class_obj,"providerName","Ljava/lang/String;");
