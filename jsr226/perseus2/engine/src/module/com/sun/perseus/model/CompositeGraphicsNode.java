/*
 * $RCSfile: CompositeGraphicsNode.java,v $
 *
...
public abstract class CompositeGraphicsNode extends ElementNode 
    ...
    //
    /**
     * Marker used to mark that the color relative property was set.
     */
    ...
    protected int markers = DEFAULT_INHERITANCE 
                            ...
                            | ViewportNode.DEFAULT_INHERITANCE 
    ...
    /**
     * @return the tight bounding box in screen coordinate space. Tight bounding
     * box is the smallest possible rectangle that includes the geometry of all
     * contained graphics elements excluding stroke.  The box coordinates are in
     * the screen coordinate space, which is connected to the current user
     * coordinate space by the matrix returned by {@link
     * org.w3c.dom.svg.SVGLocatableElement#getScreenCTM getScreenCTM} method.
     */
    ...
    public boolean isColorRelative(final int propertyIndex) {
        return isMarkerSet(propertyIndex << COLOR_RELATIVE_MARKER);
    }
    ...
    protected void setColorRelativeQuiet(final int propertyIndex, 
                                         final boolean isColorRelative) {
        if (isColorRelative) {
            setMarker(propertyIndex << COLOR_RELATIVE_MARKER);
        } else {
            clearMarker(propertyIndex << COLOR_RELATIVE_MARKER);
        }
    }
