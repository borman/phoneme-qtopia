...
public abstract class ModelNode implements EventTarget {
    ...
    /**
     * Utility method. Returns true if the input node is part of the 
    ...
    /**
     * Add an event listener that will be notified when an event of the 
     * desired type happens on this target or one of its descendants.
     *
     * An EventListener that is added to an EventTarget currently processing an
     * event is not triggered by the current actions. Duplicate instances
     * of EventListeners (registered on the same EventTarget with 
     * the same parameters) are discarded. There's even no need to remove
     * such duplicates with the removeEventListener method.
     *
     * @param type The type of event to listen to.
     * @param listener the event listener
     * @param useCapture If true, the listener will be called while in the event
     * flow capture phase. Otherwise, the listener will be called while in the
     * bubble phase. If the event's target is this target, then the listener
     * will be called while in the 'at target' phase of event flow.
     *
     * @throws DOMException with error code NOT_SUPPORTED_ERROR if useCapture is
     * true since capture phase is not supported in SVG Tiny.
    ...
    /**
     * Remove an event listener that was previously registered.
     * 
     * An EventListener that is removed from an EventTarget currently processing an
     * event is not triggered by the current actions. Nothing happens when
     * removeEventListener arguments do not identify any currently
     * registered EventListener on this EventTarget.
     *
     * @param type The type of event that was listened to.
     * @param listener The previously registered listener.
     * @param useCapture If true, the listener was listening to events in the
     * capture phase of event flow, otherwise the listener was listening to
     * events in the bubble phase.
     *
     * @throws DOMException with error code NOT_SUPPORTED_ERROR if useCapture is
     * true since capture phase is not supported in SVG Tiny.
     * @throws NullPointerException if <code>type</code> is null.
