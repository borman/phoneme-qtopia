...
public abstract class CompositeNode extends ModelNode {
    ...
    /**
     * Appends a child to the <code>Node</code>.
     *
     * @param newChild the <code>Node</code> to be appended to this
     * <code>Node</code>. This is analogue for insertBefore(newChild,null)
     * @return the added <code>Node</code>.
     *
     * @throws DOMException with HIERARCHY_REQUEST_ERR error code if
     * this type of node does not allow children of the type of the newChild
     * node, or if the appended node is an ancestor of this node
     * or this node itself, or if this node is of type Document and the DOM
     * application trys to append a second Element node.
     * @throws DOMException with WRONG_DOCUMENT_ERR error code if
     * the newChild was created from some other document than the one created
     * this node.
     * @throws DOMException with NOT_SUPPORTED_ERR error code if the newChild
     * is a child of the Document.
     * @throws DOMException with INVALID_STATE_ERR error code if the newChild 
     * node would bring the document into error state, for instance:
     * when the newChild has got a <use> element with an invalid xlink:href 
     * attribute.
    ...
    /**
     * Removes the child associated with this Node. Elements that have
     * ids cannot be deleted from the tree.
     *
     * @param oldChild the <code>Node</code> that is to be removed.
     * @return the node removed.
     * @throws DOMException with NOT_FOUND_ERR error code if the oldChild is
     * not a child of this node.
     * @throws DOMException with NOT_SUPPORTED_ERR error code if this node is
     * of type Document.
     * @throws DOMException with INVALID_ACCESS_ERR error code if the element
     * being removed or any of its decendants has non-null id.
    ...
    /**
     * Removes the given child from the parent's list of children.
     * This method does not perform any checks (doesn't check 
     * <code>oldChild</code> is a child of this Node or that it has no
     * ids etc), thus no exceptions are thrown from this method. In addition
     * this method calls setParentQuiet() so does not cause any listeners to 
     * be notified that the child is being unhooked from the tree. 
     */
    public Node removeChildQuiet(final Node oldChild) {
        ElementNode oldNode = (ElementNode) oldChild;
    ...
    /**
     * Inserts newChild right before refChild in the node's child list.
     * If the refChild is null, the newChild is appended to the end of the list.
     * If the newChild is already part of the tree, it is removed before
     * inserting.
     *
     * @param newChild the child to add
     * @param refChild the child after which the new child should be added.
     ...
     * the newChild was created from some other document than the one created
     * this node.
     * @throws DOMException with NOT_FOUND_ERR error code if the refChild is
     * not a child of this node.
     * @throws DOMException with NOT_SUPPORTED_ERR error code if the newChild
     * is a child of the Document.
     * @throws DOMException with INVALID_STATE_ERR error code if the newChild
    ...
    protected static boolean isIdBranch(final ElementNode node) {
        if (node.id != null) {
            ...
            Vector idRefs = 
                (Vector)node.ownerDocument.resolvedIDRefs.get(node.id);
            if (idRefs != null) {
                int size = idRefs.size();
                ElementNode elementNode;
                for (int i=0; i<size; i++) {
                    elementNode = (ElementNode)idRefs.elementAt(i);
                    if (!(elementNode instanceof Discard)) 
                        return true;
                }

                // All instances found above must have been Discard elements
                return false;
            }
