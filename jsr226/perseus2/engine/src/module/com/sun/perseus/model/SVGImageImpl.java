...
public class SVGImageImpl extends SVGImage {
    ...
    */
     ...
     */
    public SVGImageImpl(final DocumentNode documentNode,
    ...
    /**
     * The method creates an empty SVGImage that can be used to build a SVG
     * image.
     *
     * Please keep in the mind that a root &lt;svg&gt; element with default 
     * viewport size (100x100) is also created by the method.
     *
     * <p>The engine will invoke a hanlder (if any specifed) for each external
     * resource referenced in the document. If the handler is null, the
     * SVGImage will try to load images using the engine default 
     * implementation, but might fail while loading some of them.</p>
     *
     * <p>Note: the handler is also called by the application when xlink:href
     * attribute on &lt;image&gt; is set or changed, anyway the
     * invocation is done only if the element is hooked into the document tree.
     *
     * @param handler implementation of the ExternalResourceHandler interface
     * @see      javax.microedition.m2g.ExternalResourceHandler
     *
     * @return an newly created empty SVGImage
     */
    ...
    /**
     * The initial viewport width is the "width" value specified in 
     * the Scalable Image. The returned value is always in pixels. If the
     * width is given in percentages, the values are mapped to the 
     * default viewport size (100x100).
     * If the application changes the width of viewport explicitly,
     * then the percentages are ignored and the content is fit to new
     * viewport width.
     *
     */
    ...
    /**
     * The initial viewport height is the "height" value specified in
     * the Scalable Image. The returned value is always in pixels. If the 
     * the height is given in percentages, the values are mapped to the
     * default viewport size (100x100).
     * If the application changes the height of viewport explicitly,
     * then the percentages are ignored and the content is fit to new
     * viewport height.
     *
     */
