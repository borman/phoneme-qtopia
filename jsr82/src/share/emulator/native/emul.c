...
bt_handle_t getNextHandle(int protocol) {
    ...
    static int next = 0;
    int i;
    bt_handle_t handle = JAVACALL_BT_INVALID_HANDLE;
    ...
    if (info->flags & IN_USE) {
        if (connHandle == -1) {
            javanotify_bt_protocol_event(JAVACALL_EVENT_BT_ACCEPT_COMPLETE, 
                (bt_handle_t)myHandle, JAVACALL_FAIL);
        } else { 
            KNI_StartHandles(1);
            ...
            memcpy(info->peer_addr, JavaByteArray(addr), JAVACALL_BT_ADDRESS_SIZE);
            ...
            javanotify_bt_protocol_event(JAVACALL_EVENT_BT_ACCEPT_COMPLETE, 
                (bt_handle_t)myHandle, JAVACALL_OK);
        }
    }
...
Java_com_sun_midp_jsr82emul_ConnectionEmul_notifyOpen() {
    ...
    bt_notify_protocol_event(JAVACALL_EVENT_BT_CONNECT_COMPLETE, 
...
Java_com_sun_midp_jsr82emul_ConnectionEmul_notifyReceived() {
    ...
    if (info->flags & IN_USE) { 
        ...
        bt_notify_protocol_event(JAVACALL_EVENT_BT_RECEIVE_COMPLETE, 
...
Java_com_sun_midp_jsr82emul_ConnectionEmul_notifySent() {
    ...
    if (info->flags & IN_USE) {
        ...
        bt_notify_protocol_event(JAVACALL_EVENT_BT_SEND_COMPLETE, 
...
Java_com_sun_midp_jsr82emul_DeviceEmul_deviceDiscovered() {
    ...
    result.cod = cod;
    memcpy(result.bdaddr, JavaByteArray(buf), BTADDR_SIZE);
...
Java_com_sun_midp_jsr82emul_DeviceEmul_inquiryCompleted() {
    ...
    bt_stack_on_inquiry_complete(success? JAVACALL_TRUE : JAVACALL_FALSE);
    KNI_ReturnVoid();
}
