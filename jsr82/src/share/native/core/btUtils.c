...
/*
 * Internal utility buffer for use in Bt functions.
 * Integrity of information stored there across
...
char* jStringToChars(jstring jStringHandle)
{
    ...
    if (len < 0) {
    	...
    	pSrc = data = (jchar*)JAVAME_MALLOC(len * sizeof (jchar));
		if (data == NULL) {
	        return NULL;
		} else {
