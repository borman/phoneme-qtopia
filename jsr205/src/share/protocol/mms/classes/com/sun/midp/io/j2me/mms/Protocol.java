...
public class Protocol extends ProtocolBase {
     ...
     */
    protected native String getPhoneNumber0();
    ...
    protected void checkReceivePermission() throws InterruptedIOException {
        ...
        if (readPermission == false) {
            try {
                appPackage.checkForPermission(WMAPermission.MMS_RECEIVE);
                readPermission = true;
    ...
    public void send(Message msg) throws IOException {
        ...
        try {
            ...
            appPackage.checkForPermission(
                WMAPermission.getMmsSendPermission(
    ...
    public synchronized Message receive() throws IOException {
        ...
        try {
            ...
            length = receive0(connHandle, appID, appPackage.getId(),
                              mmsPacket);
    ...
    public synchronized Connection openPrimInternal(String name,
        ...
        if (openPermission == false) {
            try {
                appPackage.checkForPermission(WMAPermission.MMS_SERVER);
                openPermission = true;
            } catch (InterruptedException ie) {
                throw new InterruptedIOException("Interrupted while trying " +
                                                "to ask the user permission");
        ...
        try {
            connHandle = open0(host, appID, appPackage.getId());
        } catch (IOException ioe) {
            m_mode = 0;
